...
function normalizarRolApi_(rol){return String(rol||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();}
function rolCanonicoApi_(rol){const r=normalizarRolApi_(rol);if(['super admin','super administrador','superadministrador','super usuario','superusuario','super user'].includes(r))return'Super Admin';if(r==='supervisor movilidad')return'Supervisor Movilidad';if(r==='supervisor')return'Supervisor';if(r==='movilidad')return'Movilidad';if(r==='consulta movilidad')return'Consulta Movilidad';if(r==='fiscalizacion')return'Fiscalización';return String(rol||'').trim();}
function actorSesionApi_(){const s=sesionApi_();return{usuario:s.usuario||'',rol:rolCanonicoApi_(s.rol||'')};}
function rolSesionApi_(){return actorSesionApi_().rol;}
async function api(accion,datos={}){const params=new URLSearchParams({accion});Object.keys(datos).forEach(function(clave){const valor=datos[clave];if(valor!==undefined&&valor!==null&&valor!=='')params.set(clave,String(valor));});try{const respuesta=await fetch(API_URL+'?'+params.toString(),{method:'GET',cache:'no-store',redirect:'follow'});if(!respuesta.ok)return{ok:false,mensaje:'La API respondió con error '+respuesta.status+'.'};const texto=await respuesta.text();let resultado;try{resultado=JSON.parse(texto);}catch(_){return{ok:false,mensaje:'La API devolvió una respuesta no válida. Verifique la implementación de Apps Script.'};}if(resultado&&resultado.ok&&['login','guardarUsuario','actualizarUsuario','eliminarUsuario','guardarElemento','actualizarElemento','eliminarElemento','guardarZonaEstacionamiento','eliminarZonaEstacionamiento','guardarCordonRojo','eliminarCordonRojo','guardarEspacioReservado','eliminarEspacioReservado','guardarInspeccion','resolverIncidencia'].includes(accion)){const sesion=accion==='login'?resultado.usuario:sesionApi_();registrarAuditoriaCliente_(accion,datos,sesion,resultado);}return resultado;}catch(error){return{ok:false,mensaje:'No se pudo conectar con la API: '+error.message};}}
async function registrarAuditoriaCliente_(accion,datos,sesion,respuesta){try{const mapa={login:'Inicio de sesión',logout:'Cierre de sesión',guardarUsuario:'Creación de usuario',actualizarUsuario:'Modificación de usuario y permisos',eliminarUsuario:'Eliminación de usuario',guardarElemento:'Creación de elemento en mapa',actualizarElemento:'Modificación de elemento en mapa',eliminarElemento:'Remoción de elemento del mapa',guardarZonaEstacionamiento:'Creación de zona de estacionamiento',eliminarZonaEstacionamiento:'Remoción de zona de estacionamiento',guardarCordonRojo:'Gestión de cordón rojo',eliminarCordonRojo:'Eliminación de cordón rojo',guardarEspacioReservado:'Creación de espacio reservado',eliminarEspacioReservado:'Eliminación de espacio reservado',guardarInspeccion:'Creación de actuación / informe PDF',resolverIncidencia:'Cierre de incidencia de inspección'};const usuario=sesion||{};const detalle=accion==='login'?'Ingreso al sistema':accion==='logout'?'Salida del sistema':JSON.stringify(datos).slice(0,1500);const p=new URLSearchParams({accion:'registrarAuditoria',usuario:String(usuario.usuario||datos.usuario||''),nombre:String(usuario.nombre||datos.nombre||datos.inspector||''),rol:rolCanonicoApi_(usuario.rol||datos.rol||''),accionRealizada:mapa[accion]||accion,modulo:moduloAuditoria_(accion),detalle:detalle,referencia:String(respuesta.id||respuesta.codigo||respuesta.numeroSerie||datos.id||datos.elementoId||'')});await fetch(API_URL+'?'+p.toString(),{method:'GET',cache:'no-store'});}catch(_){}}
function moduloAuditoria_(accion){if(accion.includes('Usuario'))return'Usuarios';if(accion.includes('Elemento')||accion.includes('Zona')||accion.includes('Cordon')||accion.includes('EspacioReservado'))return'Movilidad';if(accion.includes('Inspeccion'))return'Fiscalización / Movilidad';if(accion==='login'||accion==='logout')return'Acceso';return'Sistema';}
...
async function apiObtenerUsuarios(){const s=actorSesionApi_();return api('obtenerUsuarios',{actorUsuario:s.usuario,actorRol:s.rol});}
async function apiGuardarUsuario(usuario){const s=actorSesionApi_();return api('guardarUsuario',Object.assign({},usuario,{actorUsuario:s.usuario,actorRol:s.rol}));}
async function apiActualizarUsuario(usuario){const s=actorSesionApi_();const datos=Object.assign({},usuario,{actorUsuario:s.usuario,actorRol:s.rol});datos.usuarioOriginal=String(usuario.usuarioOriginal||usuario.usuario||'').trim();return api('actualizarUsuario',datos);}
async function apiEliminarUsuario(usuario,passwordConfirmacion=''){const s=actorSesionApi_();return api('eliminarUsuario',{usuario:usuario,passwordConfirmacion,actorUsuario:s.usuario,actorRol:s.rol});}
async function apiObtenerElementos(){return api('obtenerElementos');}
function actorDatos_(datos){const s=actorSesionApi_();return Object.assign({},datos,{rol:datos&&datos.rol||s.rol,actorUsuario:s.usuario,actorRol:s.rol});}
...
async function apiResolverIncidencia(datos){return api('resolverIncidencia',actorDatos_(datos));}
...
async function apiObtenerEspaciosReservados(){return api('obtenerEspaciosReservados');}
async function apiGuardarEspacioReservado(espacio){return api('guardarEspacioReservado',actorDatos_(espacio));}
async function apiEliminarEspacioReservado(id){return api('eliminarEspacioReservado',actorDatos_({id:id}));}
