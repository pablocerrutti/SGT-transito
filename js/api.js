...
async function registrarAuditoriaCliente_(accion,datos,sesion,respuesta){try{const mapa={login:'Inicio de sesión',guardarUsuario:'Creación de usuario',eliminarUsuario:'Eliminación de usuario',guardarElemento:'Creación de elemento',actualizarElemento:'Modificación de elemento',eliminarElemento:'Eliminación de elemento',guardarZonaEstacionamiento:'Creación de zona de estacionamiento',eliminarZonaEstacionamiento:'Eliminación de zona de estacionamiento',guardarCordonRojo:'Creación de cordón rojo',eliminarCordonRojo:'Eliminación de cordón rojo',guardarInspeccion:'Creación de actuación / informe'};const usuario=sesion||{};const detalle=accion==='login'?'Ingreso al sistema':JSON.stringify(datos).slice(0,1500);const p=new URLSearchParams({accion:'registrarAuditoria',usuario:String(usuario.usuario||datos.usuario||''),nombre:String(usuario.nombre||datos.nombre||datos.inspector||''),rol:String(usuario.rol||datos.rol||''),accionRealizada:mapa[accion]||accion,modulo:moduloAuditoria_(accion),detalle:detalle,referencia:String(respuesta.id||respuesta.codigo||respuesta.numeroSerie||datos.id||datos.elementoId||'')});await fetch(API_URL+'?'+p.toString(),{method:'GET',cache:'no-store'});}catch(_){}}
function moduloAuditoria_(accion){if(accion.includes('Usuario'))return'Usuarios';if(accion.includes('Elemento')||accion.includes('Zona')||accion.includes('Cordon'))return'Movilidad';if(accion.includes('Inspeccion'))return'Fiscalización / Movilidad';if(accion==='login')return'Acceso';return'Sistema';}
