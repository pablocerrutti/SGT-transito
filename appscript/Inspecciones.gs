...
function guardarInspeccion(e){
  ...
  try{
    ...
    const incidencia=String(p.detalle||p.incidencia||p.descripcion||"").trim(), elementoId=String(p.elementoId||"").trim(), codigoElemento=String(p.codigoElemento||"").trim();
    ...
    if(!incidencia)return {ok:false,mensaje:"Debe describir la actuación."};
...
function generarNumeroSerieActuacion_(){
  ...
  lock.waitLock(30000);
  try{const actual=Number(props.getProperty("SGT_CONTADOR_ACTUACIONES")||"0"), siguiente=actual+1;props.setProperty("SGT_CONTADOR_ACTUACIONES",String(siguiente));return "ACT-"+String(siguiente).padStart(6,"0");}
  finally{lock.releaseLock();}
}
...
function generarPdfActuacion_(datos){
  const folder=DriveApp.getFolderById(CARPETA_ACTUACIONES_ID_), serie=String(datos["Número serie"]||"ACT"), tipo=String(datos["Tipo actuación"]||"Actuación"), titulo=tipo+" - "+serie;
  const watermark=dataUriRepositorio_(SGT_WATERMARK_URL_,"image/png"), logo=dataUriRepositorio_(SGT_LOGO_URL_,"image/png");
  const fotoHtml=construirFotosPdf_(datos["Foto/boleta URL"]||""), videoHtml=construirLinksPdf_(datos["Video URL"]||"","Abrir / descargar video"), fotoLinksHtml=construirLinksPdf_(datos["Foto/boleta URL"]||"","Abrir / descargar fotografía");
  const esc=escHtml_;
  ...
  '.watermark{position:fixed;z-index:0;top:45mm;left:8%;width:84%;height:auto;opacity:.075;}'+
  ...
  '.photo-caption{text-align:center;font-size:9px;color:#555;margin-bottom:3px;}'+
  '.photo-description{font-size:10px;text-align:center;margin:0 0 12px;color:#444;}'+
  '.links a{color:#1155cc;text-decoration:underline;}'+
  '.footer{position:fixed;z-index:3;bottom:-18mm;left:0;right:0;height:14mm;border-top:1px solid #b40000;font-size:8px;color:#555;background:#fff;padding:2mm 0 0;box-sizing:border-box;}'+
  '.footer-inner{display:flex;align-items:center;justify-content:space-between;width:100%;height:100%;}'+
  '.footer-left{display:flex;align-items:center;gap:5px;min-width:0;}'+
  '.footer-left img{height:8mm;width:auto;object-fit:contain;}'+
  '.footer-text{white-space:nowrap;}'+
  '.footer-right{display:flex;align-items:center;justify-content:flex-end;}'+
  '.footer-right img{height:10mm;width:auto;max-width:42mm;object-fit:contain;}'+
  '</style></head><body>'+
  ...
  '<table><tr><td class="label">Número de serie</td><td>'+esc(serie)+'</td></tr><tr><td class="label">Fecha y hora</td><td>'+esc(datos.Fecha||"")+'</td></tr><tr><td class="label">Responsable</td><td>'+esc(datos.Inspector||"")+'</td></tr><tr><td class="label">Usuario</td><td>'+esc(datos.Usuario||"")+'</td></tr><tr><td class="label">Rol</td><td>'+esc(datos.Rol||"")+'</td></tr></table>'+ 
  ...
  '<div class="footer-left">'+(logo?'<img src="'+logo+'">':'')+'<span class="footer-text">SGT - Sistema de Gestión de Tránsito | Documento generado automáticamente para auditoría | Serie: '+esc(serie)+'</span></div>'+ 
  '<div class="footer-right">'+(logo?'<img src="'+logo+'">':'')+'</div>'+ 
  '</div></div>'+ 
  '</body></html>';
...
function construirFotosPdf_(texto){
  ...
  urls.forEach(function(url){try{const id=extraerIdDrive_(url);if(!id)return;const file=DriveApp.getFileById(id),blob=file.getBlob(),mime=blob.getContentType()||"image/jpeg";if(mime.indexOf("image/")!==0)return;const data='data:'+mime+';base64,'+Utilities.base64Encode(blob.getBytes());partes.push('<img class="photo" src="'+data+'"><div class="photo-caption">'+escHtml_(file.getName())+'</div><div class="photo-description">'+escHtml_("Evidencia fotográfica de la actuación "+String(datosGlobalDescripcion_||""))+'</div>');}catch(error){console.error("No se pudo insertar foto en PDF:",url,error);}});return partes.join('');
}
...
function subirArchivo(e){
  ...
  try{
    ...
    let limpio=base64;if(limpio.indexOf("base64,")!==-1)limpio=limpio.split("base64,")[1];
    const mime=String(p.mimeType||"application/octet-stream"), blob=Utilities.newBlob(Utilities.base64Decode(limpio),mime,nombre), archivo=DriveApp.getFolderById(CARPETA_ACTUACIONES_ID_).createFile(blob);
...
function doPostInspecciones_(e){return doGet(e);}
