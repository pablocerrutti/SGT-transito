...
try {
  cordones = (typeof obtenerCordonesRojos === 'function')
    ? obtenerCordonesRojos({parameter:{incluirInactivos:'NO'}})
    : obtenerCordonesRojosDirectosParaInforme_();
} catch (errorCordones) {
    ...
    const datos=valores.slice(1)
      ...
      .map(function(fila){
        return {
          ...
          tipo:String(fila[2]||'Cordón Rojo').trim()||'Cordón Rojo',
      ...
      .filter(function(cordon){ return esActivoCatalogo_(cordon.activo); });
