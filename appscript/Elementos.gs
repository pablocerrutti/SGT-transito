...
function obtenerPrefijo(tipo) {
  const prefijos = {'Semáforo':'SEM','Radar':'RAD','Cruce Peatonal':'CRU','Lomo de Burro':'LOM','Cartel':'CAR','Señal Vertical':'SVE','Señal Horizontal':'SHO','Cámara':'CAM','Otro':'OTR'};
...
function obtenerElementos() {
  try {
    ...
    const lista = datos.filter(function(f){ return String(f[0] || '').trim() !== ''; }).map(function(f) {
      return {id:f[0],codigo:f[1],tipo:f[2],serie:f[3],nombre:f[4],descripcion:f[5],latitud:f[6],longitud:f[7],direccion:f[8],estado:f[9],caracteristicas:f[10],fechaAlta:f[11],usuarioAlta:f[12],fechaModificacion:f[13],usuarioModificacion:f[14],activo:f[15]};
    });
...
function guardarElemento(e) {
  ...
  try {
    ...
    sh.appendRow([generarID('EL'),codigo,tipo,serie,String(p.nombre || '').trim(),String(p.descripcion || '').trim(),coordenadas.latitud,coordenadas.longitud,String(p.direccion || '').trim(),String(p.estado || 'Activo').trim(),String(p.caracteristicas || '').trim(),ahora(),usuario,'','','SI']);
