...
function esTipoObservaciones_(tipo) {
  ...
  return valor === 'otro' || valor === 'otros' || valor === 'observacion' || valor === 'observaciones';
}

function nombreTipoElemento_(tipo) {
  return esTipoObservaciones_(tipo) ? TIPO_OBSERVACIONES : String(tipo || '').trim();
}
...
function obtenerPrefijo(tipo) {
  ...
  return prefijos[nombreTipoElemento_(tipo)] || 'OTR';
}

function obtenerSiguienteSerie(tipo) {
  return obtenerSiguienteSerieEnHoja_(hoja('Elementos'), tipo);
}
...
function obtenerSiguienteSerieEnHoja_(sh, tipo) {
  ...
  const tipoNormalizado = nombreTipoElemento_(tipo);
  let mayor = 0;
  datos.forEach(function(fila) {
    const tipoFila = nombreTipoElemento_(fila[0]);
    if (tipoFila !== tipoNormalizado) return;
...
function obtenerElementos() {
  try {
    const sh = hoja('Elementos'), ultimaFila = sh.getLastRow();
    ...
    return {ok:false,mensaje:'No fue posible obtener los elementos: '+error.message};
  }
}

function guardarElemento(e) {
  const bloqueoRol = bloquearMutacionSupervisorMovilidad_(e,'Este rol no puede crear elementos del mapa.');
  if (bloqueoRol) return bloqueoRol;
  const p = (e && e.parameter) || {};
  const tipo = nombreTipoElemento_(String(p.tipo || '').trim() || TIPO_OBSERVACIONES);
...
function actualizarElemento(e) {
  const bloqueoRol = bloquearMutacionSupervisorMovilidad_(e,'Este rol no puede modificar elementos del mapa.');
  if (bloqueoRol) return bloqueoRol;
  const p = (e && e.parameter) || {}, id = String(p.id||'').trim();
  ...
  try {
    ...
    return {ok:false,mensaje:'No fue posible actualizar el elemento: '+error.message};
  }
}

function eliminarElemento(e) {
  const bloqueoRol = bloquearMutacionSupervisorMovilidad_(e,'Este rol no puede eliminar elementos del mapa.');
  if (bloqueoRol) return bloqueoRol;
  ...
  try {
    ...
    if (fila === -1) return {ok:false,mensaje:'Elemento no encontrado.'};
    sh.deleteRow(fila);
    return {ok:true,mensaje:'Elemento eliminado.'};
  } catch(error) {
    return {ok:false,mensaje:'No fue posible eliminar el elemento: '+error.message};
  }
}
...
function asegurarColumnasTerritoriales_(sh) {
  if (sh.getLastColumn() < 19) sh.getRange(1,17,1,3).setValues([['Ciudad','Localidad','Zona']]);
}
