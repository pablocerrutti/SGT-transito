...
}else if(!["super admin","supervisor","fiscalizacion","movilidad"].includes(rol)){
...
function cargarInspector(){
  ...
  const esMovilidad=rol==="movilidad";
  if(inspector) inspector.value=usuario.nombre||usuario.usuario||"";
  if(beta) beta.value=esMovilidad ? "No posee" : (usuario.beta||"");
  if(rol==="fiscalizacion" && !usuario.beta){
    mensaje("El inspector no tiene Número Beta asignado. Contacte al administrador.","error");
  }
}

function actualizarCamposPorRol(){
  const matricula=document.getElementById("matricula");
  const nombre=document.getElementById("nombreInfractor");
...
async function guardar(ev){
  ...
  try{
    ...
    const tipo=document.getElementById("tipoActuacion").value;
    if(!tipo) throw new Error("Debe seleccionar el tipo de actuación.");

    const requiereFoto=tipo==="Multa"||tipo==="Apercibimiento";
    const esMovilidad=rol==="movilidad";
    ...
    const datos={
      beta:esMovilidad?"No posee":(usuario.beta||""),
      inspector:usuario.nombre||usuario.usuario||"",
      usuario:usuario.usuario||"",
      rol:rol,
      matricula:document.getElementById("matricula").value.trim().toUpperCase(),
      tipoActuacion:tipo,
      numeroBoleta:document.getElementById("numeroBoleta").value.trim(),
      nombreInfractor:document.getElementById("nombreInfractor").value.trim(),
    ...
    console.log("Guardando actuación:",datos);
    ...
    mensaje(error.message||"No fue posible guardar la actuación.","error");
  }finally{
...
async function cargar(){
  try{
    ...
    (r.datos||[]).forEach(i=>{
      ...
      x.innerHTML=`
        <strong>${esc(i.numeroSerie||i.id||"")} - ${esc(i.matricula||"Sin matrícula")} - ${esc(i.tipoActuacion||"")}</strong>
        <p>${esc(i.detalle||"")}</p>
        <small>${esc(i.inspector||"")} ${i.beta?" - Beta "+esc(i.beta):""} - ${esc(i.rol||"")} - ${esc(i.fecha||"")}</small>
        ${i.pdfUrl?`<br><a target="_blank" rel="noopener noreferrer" href="${esc(i.pdfUrl)}">Reporte PDF</a>`:""}
        ${i.videoUrl?`<br><a target="_blank" rel="noopener noreferrer" href="${esc(i.videoUrl)}">Video</a>`:""}
        ${i.documentoUrl?`<br><a target="_blank" rel="noopener noreferrer" href="${esc(i.documentoUrl)}">Fotografía / boleta</a>`:""}
      `;
      lista.appendChild(x);
    });
  }catch(error){
