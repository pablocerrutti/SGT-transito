...
(function(){
  ...
  function capturarPunto(e){
    ...
    if(puntos.length>=2){
      ...
      estado('Dos puntos definidos. Haga clic en "Guardar espacio reservado".');
      confirmarGuardar();
    }else estado('Punto 1 registrado. Seleccione el segundo punto.');
  }
  ...
  function confirmarGuardar(){
    ...
    btn.type='button';btn.id='btnGuardarEspacioReservado';btn.className='btn-guardar-geometria';
    btn.innerHTML='<i class="fa-solid fa-floppy-disk"></i> Guardar espacio reservado';
    btn.addEventListener('click',guardar);
    estadoEl.appendChild(document.createTextNode(' '));estadoEl.appendChild(btn);
  }
  ...
  async function guardar(){
    ...
    try{
      const u=usuario();
      const r=await api('guardarEspacioReservado',{coordenadas:JSON.stringify(puntos),nombre:'ESPACIO RESERVADO',descripcion:'Espacio reservado definido mediante dos puntos en el mapa.',estado:'Activo',usuario:u.usuario||u.nombre||'admin',rol:u.rol||''});
  ...
  function cancelarDibujo(){
    if(mapaDisponible())mapa.off('click',capturarPunto);
    dibujando=false;puntos=[];
    if(lineaTemporal){lineaTemporal.remove();lineaTemporal=null;}
    const boton=document.getElementById('btnNuevoEspacioReservado');
    const cancelar=document.getElementById('btnCancelarEspacioReservado');
    if(boton)boton.style.display='inline-block';if(cancelar)cancelar.style.display='none';
    const guardarBtn=document.getElementById('btnGuardarEspacioReservado');if(guardarBtn)guardarBtn.remove();
    estado('');
  }
  ...
  function instalar(){
    if(instalado)return;
    if(!mapaDisponible()){setTimeout(instalar,200);return;}
    instalado=true;crearUI();
    ['filtroTipo','tipo'].forEach(function(id){
      const select=document.getElementById(id);if(!select)return;
      if(!Array.from(select.options).some(o=>norm(o.value||o.text)===norm(TIPO)))select.add(new Option(TIPO,TIPO));
    });
    if(typeof seleccionarUbicacion==='function'){
      mapa.off('click',seleccionarUbicacion);
      mapa.on('click',function(e){if(dibujando)return;seleccionarUbicacion(e);});
    }
